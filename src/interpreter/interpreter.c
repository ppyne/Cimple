...
/* Helper: base name of a built-in exception type, NULL if not a built-in exception */
...
int interp_struct_is_subtype(Interp *ip, const char *actual, const char *expected) {
    ...
    /* Walk user-defined inheritance chain */
    ...
    /* Continue through built-in exception hierarchy if we landed on one */
